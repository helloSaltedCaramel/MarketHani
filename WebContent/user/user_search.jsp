...
function sort(search, how) {
	
	$('.top_line form').attr('action', 'user_search.do?search=' + search +  '&sort=' + how); 
	$('.sort').submit();
...
<body>
	...
	<div align="center">
		...
		<div class="article" align="center">
			<div class="category"><span>제품 검색 : ${search }</span></div>
			...
			<div class="pagination" align="center">
				...
				<c:if test="${page == 1}">
					<a class="first" href="user_search.do?search=${search}&sort=${sortBy}&page=1"><span></span></a>
					<a class="prev" href="user_search.do?search=${search}&sort=${sortBy}&page=1"><span></span></a>
				</c:if>
			
				<c:if test="${page != 1}">
					<a class="first" href="user_search.do?search=${search}&sort=${sortBy}&page=1"><span></span></a>
					<a class="prev" href="user_search.do?search=${search}&sort=${sortBy}&page=${page - 1 }"><span></span></a>
				</c:if>
			
				<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
					<c:if test="${i == page }">
						<a id="on" href="<%=request.getContextPath() %>/user_search.do?search=${search}&sort=${sortBy}&page=${i }"><span>${i }</span></a>
					</c:if>
					
					<c:if test="${i != page }">
						<a href="<%=request.getContextPath() %>/user_search.do?search=${search}&sort=${sortBy}&page=${i }"><span>${i }</span></a>
					</c:if>
				</c:forEach>
				
				<c:if test="${page == allPage}">
					<a class="next" href="user_search.do?search=${search}&sort=${sortBy}&page=${allPage }"><span></span></a>
					<a class="last" href="user_search.do?search=${search}&sort=${sortBy}&page=${allPage }"><span></span></a>
				</c:if>	
				
				<c:if test="${page != allPage}">
					<a class="next" href="user_search.do?search=${search}&sort=${sortBy}&page=${page + 1 }"><span></span></a>
					<a class="last" href="user_search.do?search=${search}&sort=${sortBy}&page=${allPage }"><span></span></a>
				</c:if>
