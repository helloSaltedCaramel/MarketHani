...
public class AdminProdUpdatingAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String p_img_name = multi.getParameter("old_p_image");
		if (multi.getFile("p_img") != null) {
			p_img_name = settingFile(uploadPath, multi.getFile("p_img"));
		}

		String p_contents_name = multi.getParameter("old_p_contents");
		if (multi.getFile("p_contents") != null) {
			p_contents_name = settingFile(uploadPath, multi.getFile("p_contents"));
		}

		String p_contents_spec_name = multi.getParameter("old_p_contents_spec");
		if (multi.getFile("p_contents_spec") != null) {
			p_contents_spec_name = settingFile(uploadPath, multi.getFile("p_contents_spec"));
		}

		System.out.println("[AdminProdUpdatingAction : Product Update]");
		System.out.println("p_image : " + p_img_name);
		System.out.println("p_contents : " + p_contents_name);
		System.out.println("p_contents_spec_name : " + p_contents_spec_name);
		...
		int result = ProductDAO.getInstance().updateProduct(dto);
		...
		if (result == -1) {
			...
			oldDeleteFile(uploadPath, p_img_name, multi.getFile("p_img"));
			oldDeleteFile(uploadPath, p_contents_name, multi.getFile("p_contents")); 
			oldDeleteFile(uploadPath, p_contents_spec_name, multi.getFile("p_contents_spec"));
		}

		ActionForward forward = new ActionForward();
		forward.setPath("user_product_view.do?p_num=" + multi.getParameter("p_num"));
	...
	private void oldDeleteFile(String uploadPath, String fileName, File comparePath) {
		if (comparePath != null) {
			return;
		}

		String directory = uploadPath + "/" + fileName;
